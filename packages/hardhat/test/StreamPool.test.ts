...
import { Stream } from "../typechain-types/contracts/Stream";
import { ERC20Mock } from "../typechain-types/contracts/ERC20Mock";
import { PoolWrapper } from "../typechain-types/contracts/PoolWrapper";
...
describe("Stream Pool Creation", function () {
    ...
    it("should store pool configuration during stream creation", async function () {
        ...
        const { contracts, timeParams, accounts } = await loadFixture(stream().streamOut(streamOutAmount).poolOutSupply(poolOutSupplyAmount).build());
    ...
    it("should create pool and transfer tokens when stream is finalized", async function () {
        const { contracts, timeParams, accounts, config } = await loadFixture(stream().streamOut(streamOutAmount).poolOutSupply(poolOutSupplyAmount).build());
        ...
        // Finalize the stream
        let tx = await contracts.stream.connect(accounts.creator).finalizeStream();
        await tx.wait();
