...
describe("Stream Withdraw", function () {
  ...
  describe("Overflow scenarios on withdraw", function () {
    it("Should revert with Panic(0x11) when partial withdraw overflows share multiplication (ERC20)", async function () {
      const { contracts, timeParams, accounts } = await loadFixture(stream().timeParams(10, 10, 100).build());
      ...
      // Subscribe with subscriber1 100 tokens
      ...
      // Increase time to 5% of stream duration
      await ethers.provider.send("evm_setNextBlockTimestamp", [timeParams.streamStartTime + 5]);
      await ethers.provider.send("evm_mine", []);
      ...
      // increase time to 10% of stream duration
      await ethers.provider.send("evm_setNextBlockTimestamp", [timeParams.streamStartTime + 10]);
      await ethers.provider.send("evm_mine", []);
      ...
      // Increase time to 20% of stream duration
      await ethers.provider.send("evm_setNextBlockTimestamp", [timeParams.streamStartTime + 20]);
      await ethers.provider.send("evm_mine", []);

      // Subscribe with subscriber2 100 tokens
      await contracts.inSupplyToken
        .connect(accounts.subscriber2)
        .approve(contracts.stream.getAddress(), subscriptionAmount);
      await contracts.stream.connect(accounts.subscriber2).subscribe(subscriptionAmount);

      // Increase time to 30% of stream duration
      await ethers.provider.send("evm_setNextBlockTimestamp", [timeParams.streamStartTime + 30]);
      await ethers.provider.send("evm_mine", []);

      // Withdraw 50 tokens
      await contracts.stream.connect(accounts.subscriber2).withdraw(withdrawAmount);

      // Increase time to 40% of stream duration
      await ethers.provider.send("evm_setNextBlockTimestamp", [timeParams.streamStartTime + 40]);
      await ethers.provider.send("evm_mine", []);
      ...
      // Increase time to 50% of stream duration
      await ethers.provider.send("evm_setNextBlockTimestamp", [timeParams.streamStartTime + 50]);
      await ethers.provider.send("evm_mine", []);
